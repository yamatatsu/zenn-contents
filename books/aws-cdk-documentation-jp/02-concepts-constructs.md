...
- **id** – この Scope 内で一意である必要がある [Identifier](./07-identifiers.md) 。 Identifier は、現在の Construct 内で定義されているすべての名前空間として機能し、[resource name](./06-resources.md#Physical-names)や CFn の論理 ID などの一意の ID を割り当てるために使用されます。
...
:::message
**Note**
上記の例で作成するバケットの名前では`MyFirstBucket`ありません。新しい Construct に与えられた logical identifier が設定されます。詳細については、[Physical names](./06-resources.md#Physical-names) を参照してください。
:::
...
AWS Construct Library の一般的な API パターンについては、[resources](./06-resources.md)を参照してください。
...
AWS CDK は、Construct tree 内のすべての node にアクセスし、各 node で操作を実行する簡単な方法を提供します。[Aspects](./15-aspects.md)を参照してください。
